...
export function buildProtocolPrompt(state, options = {}) {
    ...
    if (mode === 'NATIVE') {
        const protocol = [
            ...
            'Allowed native data lines only: actor.set|ID|field|value ; actor.create|ID|field|value ; scene.set|field|value ; scene.position|ID|value ; relation.set|A|B|bond|value ; relation.set|A|B|sparks|value ; relation.set|A|B|grudge|value ; residue.set|RID|field|value ; residue.remove|RID. Emit only fields changed this turn; a zero-delta NORMAL patch still advances ct exactly once.',
    ...
    const protocol = [
        ...
        'Allowed data lines only: actor.set|ID|field|value ; actor.create|ID|field|value ; scene.set|field|value ; scene.position|ID|value ; relation.set|A|B|bond|value ; relation.set|A|B|sparks|value ; relation.set|A|B|grudge|value ; residue.set|RID|field|value ; residue.remove|RID. The value is the entire remainder of its one line and needs no quoting or escaping. Emit only fields changed this turn; never restate whole records.',
