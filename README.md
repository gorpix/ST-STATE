...
- A state-less Hybrid Native chat initializes identity-only `US` and active-card actor rows at `ct 0`, so the first generated reply can use valid IDs without a Legacy bootstrap turn.
